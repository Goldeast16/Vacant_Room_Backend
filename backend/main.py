...
async def get_buildings():
    # MongoDB에서 모든 건물 목록을 가져오는 쿼리
    buildings = rooms_collection.distinct("building")
    return buildings

# 강의실 목록 가져오기
@app.get("/api/rooms", response_model=List[RoomOut])
async def get_rooms(building: str, hour: int, minute: int, weekday: str, floor: Optional[str] = None):  # floor는 Optional[str]로 변경
    ...
    if floor and floor != "all":  # floor가 "all"이 아닐 경우만 필터를 적용
        query["floor"] = int(floor)  # floor는 정수로 변환하여 사용
...
# 건물에 해당하는 층 목록 가져오기
@app.get("/api/floors", response_model=List[int])
